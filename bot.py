...
ADMIN_IDS = [2005443219]  # Replace with your Telegram ID
...
def main():
    ...
    # Add job queue for daily Bible verse
    job_queue = application.job_queue
    if job_queue:
        try:
            # Send first verse in 60 seconds, then daily
            job_queue.run_repeating(send_daily_verse, interval=86400, first=60)
            print("✅ Daily Bible verse scheduler enabled")
        except Exception as e:
            print(f"⚠️  Could not start daily verse scheduler: {e}")
    ...
    print("✅ 3 Templates available")
    print(f"👑 Admin access for: {ADMIN_IDS}")
    print("📊 Database: bot_database.db")
    print(f"📺 YouTube: {DEVELOPER_INFO['youtube']}")
